...
{
    public class MeshClassificationFrackingFixed : MonoBehaviour
    {
        /// <summary>
        ...
        void OnMeshesChanged(ARMeshesChangedEventArgs args)
        {
            ...
            if (args.updated != null)
            {
                ...
                /*foreach (var mf in args.updated) {
                    if (m_MeshFrackingMap.ContainsKey(ExtractTrackableId(mf.name))) {
                        m_UpdateMeshAction(mf);
                    } else {
                        m_BreakupMeshAction(mf);
                    }
                }*/
            }
        ...
        {
            var meshSubsystem = m_MeshManager.subsystem;
            ...
            var faceClassifications = GetFaceClassifications(meshId, Allocator.Persistent);
        ...
        static NativeArray<ARMeshClassification> GetFaceClassifications(TrackableId meshId, Allocator allocator) 
        {
            #if UNITY_EDITOR && AR_FOUNDATION_REMOTE_INSTALLED
            if (Application.isEditor) 
            {
                return ARFoundationRemote.Runtime.ARKitMeshingExtensions.GetFaceClassifications(meshId, allocator);
            }
            #endif

            return ARKitMeshSubsystemExtensions.GetFaceClassifications(null, meshId, allocator);
        }
